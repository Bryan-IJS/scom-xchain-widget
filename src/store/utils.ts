...
import { BigNumber, ERC20ApprovalModel, IClientSideProvider, IClientWalletConfig, IERC20ApprovalEventOptions, INetwork, Wallet } from '@ijstech/eth-wallet';
import { EventId, IExtendedNetwork, SITE_ENV, TokenMapType } from '../global/index';
import getNetworkList from '@scom/scom-network-list';
import { CoreContractStore } from './data/index';
...
export function determineRealOrderStatus(expire: number | BigNumber, fromChainStatus: VaultOrderStatus, toChainStatus: VaultOrderStatus): VaultOrderStatus {
  switch (toChainStatus) {
    ...
    case VaultOrderStatus.NotSpecified://0
      if (fromChainStatus == VaultOrderStatus.Pending && new BigNumber(new Date().getTime()).shiftedBy(-3).gte(expire)) {
        return VaultOrderStatus.Expired;
      } else {
        return toChainStatus
      }
    case VaultOrderStatus.RequestCancel:
      if (fromChainStatus == VaultOrderStatus.Cancelled) return VaultOrderStatus.Cancelled;
    default:
      return toChainStatus;
  }
}
